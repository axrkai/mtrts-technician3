...
DROP TABLE IF EXISTS wo_safety_completions;
DROP TABLE IF EXISTS wo_safety_checks;
...
-- Safety pre-flight checks template (global, not per category).
...
-- Completion state of each safety check per work order instance.
...
-- Seed: default safety pre-flight checks
INSERT INTO wo_safety_checks (check_text, is_mandatory, sort_order) VALUES
  ...
  ('Verify tools are in good working condition', 0, 5),
...
-- General checklist items (checklist_id = 5) - with verification types for auto-checking
...
-- Progress notes added during work order execution.
