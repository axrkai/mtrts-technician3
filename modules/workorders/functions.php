...
function get_wo_by_id(PDO $pdo, int $id): array|false {
    $stmt = $pdo->prepare("
        SELECT w.*,
               ...
               r_q.role_name    AS queue_role_name,
        ...
        LEFT JOIN roles            r_q    ON w.assigned_role_id = r_q.role_id
...
function get_all_technicians(PDO $pdo): array {
    // Includes IT staff + technicians for assignment (use role_name to avoid brittle role_id assumptions)
    $stmt = $pdo->prepare("
        SELECT u.user_id, u.full_name
        FROM users u
        JOIN roles r ON u.role_id = r.role_id
...
function get_queue_roles(PDO $pdo): array {
    // Roles that can take/claim work orders.
    $stmt = $pdo->prepare("
        SELECT role_id, role_name
        FROM roles
        WHERE role_name IN ('it_staff', 'technician', 'it_manager')
        ORDER BY FIELD(role_name, 'it_staff', 'technician', 'it_manager')
    ");
    $stmt->execute();
    return $stmt->fetchAll();
}

function get_default_queue_role_id(PDO $pdo): ?int {
    // Default queue destination for newly created work orders.
    $stmt = $pdo->prepare("SELECT role_id FROM roles WHERE role_name = 'it_staff' LIMIT 1");
    $stmt->execute();
    $id = $stmt->fetchColumn();
...
function create_work_order(PDO $pdo, array $d): int {
    ...
    $pdo->prepare("
        INSERT INTO work_orders
            (wo_number, ticket_id, wo_type, assigned_role_id,
...
function update_work_order(PDO $pdo, int $id, array $d): void {
    $pdo->prepare("
        UPDATE work_orders SET
            wo_type=?, assigned_role_id=?, assigned_to=?, assigned_by=?,
...
function sanitize_wo_post(array $post, int $user_id): array {
    ...
    return [
        ...
        'assigned_role_id' => $int('assigned_role_id'),
